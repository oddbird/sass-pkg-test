...
const files = fs
  ...
  .filter((file) => fileExtension(file) === '.json');
...
const rootKeys = {
  sass: database
    .filter((pkg) => 'sass' in pkg)
    .map((pkg) => rootKeyMap(pkg, 'sass')),
  style: database
    .filter((pkg) => 'style' in pkg)
    .map((pkg) => rootKeyMap(pkg, 'style')),
  styles: database
    .filter((pkg) => 'styles' in pkg)
    .map((pkg) => rootKeyMap(pkg, 'styles')),
};
